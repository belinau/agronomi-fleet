...
Stays awake forever; receives pump_on/pump_off commands and sends
...
_pump_state = False  # Current pump relay state (True = relay energised)
_cmd_counter = 0  # Auto-increment for unlabelled command tracking
...
def _set_pump(on):
    global _pump_state
    _pump_pin.value(1 if on else 0)
    _pump_state = bool(on)
    _log("Pump relay " + ("ON" if on else "OFF"))


def _suspend_ble_interface(rns):
    """See m_reticulum/sn_support/firmware/main.py for full rationale."""
...
def _send_ack(hub_identity, cmd, cmd_id, status, error=None):
    """Send a command-result Packet back to the hub."""
    try:
...
def _handle_hardware_command(cmd, cmd_id):
    error = None
    if cmd == "pump_on":
        _set_pump(True)
    elif cmd == "pump_off":
...
def _on_packet_received(data, packet):
    ...
    try:
        ...
        if cmd == "execute":
            ...
            _cmd_counter += 1
            _handle_hardware_command(cmd_type, cmd_id)
            return

        # Direct command (no "execute" wrapper)
        if cmd in ("pump_on", "pump_off"):
            cmd_id = fields.get("cmd_id", _cmd_counter)
            _cmd_counter += 1
            _handle_hardware_command(cmd, cmd_id)
...
async def main():
    ...
    # Wait briefly for hub announce so the first fw_check has somewhere to land
    ...
    # Firmware now operational — confirm this build.
    try:
