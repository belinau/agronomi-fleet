...
Lifecycle:
  ...
  3. Read sensors (air temp + humidity + battery)
...
def _handle_hardware_command(cmd):
    # SN-AIR is a sensor node — no relays or actuators attached.  Stub
...
def _build_telemetry_fields(readings, interface_name):
    ...
    if bat is not None:
        fields["bat"] = bat
    temp = readings.get("air_temp_c")
    if temp is not None and readings.get("air_temp_valid", True):
        fields["temp"] = temp
    hum = readings.get("air_humidity_pct")
    if hum is not None and readings.get("air_humidity_valid", True):
        fields["hum"] = hum
    return fields
...
async def main():
    ...
    # Sensors come AFTER hub registration. Wrap defensively — a bad pin /
    # missing config / disconnected DHT22 must not crash main(). The async
    # DHT22 reader handles its own 2 s stabilisation and retry logic.
    readings = {}
    _log("Reading sensors...")
    try:
        readings = await read_all_async(config)
        if readings.get("air_temp_c") is not None:
            _log("Air temp:      {:.1f}°C".format(readings["air_temp_c"]))
        if readings.get("air_humidity_pct") is not None:
            _log("Air humidity:  {:.1f}%".format(readings["air_humidity_pct"]))
        if readings.get("battery_v") is not None:
