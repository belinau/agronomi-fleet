"""µReticulum — Greenhouse Actuator Node Sensors (AN-GREENHOUSE-01)

Minimal sensor drivers for the greenhouse node.  Greenhouse nodes have no
environmental sensors — the only "sensor" reading is the battery voltage
(if the board has a voltage divider) and the current actuator states
(tracked in main.py, not here).
"""
...
def read_all(config):
    ...
    For greenhouse nodes, this is just battery voltage (if HAS_BAT_RESISTORS
    is enabled).  Actuator state (vent_open, shade_pct, fan_on) is tracked
    in main.py and included in telemetry there.
